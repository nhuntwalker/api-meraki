...
with open(os.path.join(os.pardir, "api/api_credentials.json"), "r") as creds:
    ...
    with open(os.path.join(os.pardir, 'api/') + 'api_network_ids.csv', "r") as networks:
        ...
        for row in reader:
            networkid = "N_000000000000000" #row[0]


            # GET - List Air Marshal scan results from a network
            query = requests.get('https://api.meraki.com/api/v0/networks/' + networkid + '/airMarshal',
                                 headers={'X-Cisco-Meraki-API-Key': apikey, 'Content-Type': 'application/json'})
            print(query.status_code)

            with open(os.path.join(os.pardir, 'data/')+networkid+'_air_marshal.json', "w") as f:
                    answer = query.json()
