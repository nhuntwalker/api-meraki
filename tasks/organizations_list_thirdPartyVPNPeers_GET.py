...
with open(os.path.join(os.pardir, "api/api_credentials.json"), "r") as creds:
    ...
    query = requests.get('https://api.meraki.com/api/v0/organizations/' + orgid + '/thirdPartyVPNPeers',
                         headers={'X-Cisco-Meraki-API-Key': apikey, 'Content-Type': 'application/json'})
    print(query.status_code)

    with open(os.path.join(os.pardir, 'data/')+orgid+'_thirdPartyVPNPeers.json', "w") as f:
            answer = query.json()
